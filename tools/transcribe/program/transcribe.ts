...
export async function transcribe(path: string, options: TranscribeOptions) {
  ...
  if (!path.endsWith('.md')) {
    console.error('Error: Path must be a markdown file');
    process.exit(1);
  }
  ...
  if (content.length !== 0) {
    console.error('Error: Post content is not empty');
    process.exit(1);
  }
  if (!provider) {
  ...
  const newContent =
    `<strong>Auto-generated with ${transcriptService.getName()}. Review and edit.</strong>` +
    '\n\n' +
    transcript;
